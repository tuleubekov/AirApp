package com.berg.airapp.day5.presentation
...
private fun MetricsCard(state: ModelTabState) {
    Card(
        ...
        Row(
            ...
            MetricItem(label = "⏱ Время", value = formatDuration(state.durationMs))
            MetricItem(label = "📊 Токены", value = "${state.inputTokens}+${state.outputTokens}")
            MetricItem(label = "💰 Цена", value = formatCost(state.costUsd))
        }
    }
}
